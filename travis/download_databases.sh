...
then
    ...
    wget \
        http://ftp.ebi.ac.uk/pub/databases/metagenomics/VirSorter_data_viral/virsorter-data-v2.tar.gz -O virsorter-data-v2.tar.gz \
...
then
  mkdir -p "$1/vpHMM" && \
  wget \
    http://ftp.ebi.ac.uk/pub/databases/metagenomics/vpHMM_database.tar.gz -O vpHMM_database.tar.gz \
    --progress=bar:force:noscroll && \
...
then
  wget \
    http://ftp.ebi.ac.uk/pub/databases/metagenomics/viral-pipeline/ete3_ncbi_tax.sqlite.gz -O ete3_ncbi_tax.sqlite.gz \
    --progress=bar:force:noscroll && \
...
then
  wget \
    http://ftp.ebi.ac.uk/pub/databases/metagenomics/viral-pipeline/vphmm_2020-01-29.pickle -O "$1/vphmm_2020-01-29.pickle" \
    --progress=bar:force:noscroll
else
